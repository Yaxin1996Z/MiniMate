"""Agent —— 单 Agent 核心：chat / react / plan 三种模式 + 交互观测

后续多 Agent 编排（协作 / 调度）在 agent/ 包内扩展。
"""
...
from ..memory import MemoryManager, ToolMemory
...
class Agent:
    ...
    def __init__(
        ...
        self.history_compactor = history_compactor or ConversationHistoryCompactor()
        self.tool_memory = tool_memory or ToolMemory()
    ...
    def _run_react(self, task: str, context: str = "") -> str:
        ...
        loop_id = self.tool_memory.new_loop()  # 当前 react 循环，工具记忆按此隔离
        try:
    ...
    def _run_react_fc(self, messages: list[dict], loop_id: str) -> str:
        ...
        for step in range(1, self.max_steps + 1):
            if self.history_compactor:
                self.history_compactor.compact_if_needed(messages)
            ...
            if tool_calls:
                ...
                for tc in tool_calls:
                    ...
                    repeat_hint = self._check_repeat(name, args, loop_id)
                    if repeat_hint:
                        observation = repeat_hint
                        self.tool_memory.record(
                            name, args, observation,
                            ok=False, step=step, loop_id=loop_id,
                        )
                    else:
                        t0 = time.time()
                        observation = self.tools.execute(name, **args)
                        if self.memory:
                            self.memory.add_tool_result(name, observation)
                        self.tool_memory.record(
                            name, args, observation,
                            ok=not observation.startswith(("[可重试]", "[不可重试]", "[API 错误]")),
                            step=step, loop_id=loop_id,
    ...
    def _run_react_text(self, messages: list[dict], loop_id: str) -> str:
        ...
        for step in range(1, self.max_steps + 1):
            ...
            if parsed["action"]:
                ...
                repeat_hint = self._check_repeat(
                    parsed["action"], parsed["action_input"], loop_id
                )
                if repeat_hint:
                    observation = repeat_hint
                    self.tool_memory.record(
                        parsed["action"], parsed["action_input"], observation,
                        ok=False, step=step, loop_id=loop_id,
                    )
                else:
                    ...
                    if self.memory:
                        self.memory.add_tool_result(parsed["action"], observation)
                    self.tool_memory.record(
                        parsed["action"], parsed["action_input"], observation,
                        ok=not observation.startswith(("[可重试]", "[不可重试]", "[API 错误]")),
                        step=step, loop_id=loop_id,
    ...
    def _check_repeat(
        ...
        """重复调用检测：当前 react 循环内相同 (工具, 参数) 连续尝试 >= 2 次时拦截

        args 支持 dict（FC 通道）或字符串（文本协议通道）。
        命中时返回提示文本（作为 Observation 回灌给模型），未命中返回 None。
        """
        count = self.tool_memory.repeated_count(tool_name, args, loop_id)
        if count >= 1:
            logger.warning(
                "重复调用拦截 tool=%s args=%s count=%d",
                tool_name, args, count + 1,
            )
            return (
                f"[重复调用] 工具 {tool_name} 使用相同参数已连续尝试 {count + 1} 次未成功。"
